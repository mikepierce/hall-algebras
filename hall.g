################################################################################
## hall.g
## Functions for computing the Hall algebra of a quiver algebra generated by QPA
...
local ext, basis, scalars, space;
    ...
    # Get a basis for Ext(M,N) in terms of pullback maps (see documentation)
...
HallNumber := function(M,N,Y)
    ...
    if Size(FieldOfPathAlgebraModule(M))=infinity  then
        Error("this function only works with path algebra modules over finite fields,");
    fi;
...
## WTF is AllModulesOfLengthPlusOne???
